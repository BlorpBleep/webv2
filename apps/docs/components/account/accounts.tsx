...
export default function Accounts() {
  ...
  useEffect(() => {
    const checkSessionAndFetchAccounts = async () => {
      const { data: sessionData, error: sessionError } = await supabase.auth.getSession();

      if (sessionError || !sessionData?.session) {
        router.push("/auth"); // Redirect to the authentication page if session is invalid
        return; // Stop further execution if no session is found
      }

      fetchAccounts(sessionData.session.user.id);
    };
  ...
  const addNewAccount = async () => {
    ...
    if (createError) {
      ...
      await fetchAccounts(userId); // Reload the accounts after adding a new one
  ...
  const deleteAccount = async () => {
    ...
    if (deleteError) {
      ...
      await fetchAccounts(selectedAccount.user_id); // Reload the accounts after deletion
    }
  ...
  return (
    <div className="max-w-4xl mx-auto text-gray-900 dark:text-white">
      ...
      <h2 className="text-l font-medium mb-4 text-gray-700 dark:text-gray-400">Account list</h2>
